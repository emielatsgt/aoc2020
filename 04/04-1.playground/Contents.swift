...
func parse(lines: [String]) -> [Passport] {
    ...
    for line in lines {
        // Empty line starts a new passport
        if line.isEmpty {
            var passport: Passport = [:]
            
            if !temp.isEmpty && !temp[0].isEmpty {
                
                for item in temp {
                    let keyvalue = item.trimmingCharacters(in: .whitespaces).components(separatedBy: .whitespaces)
...
var passports = parse(lines: lines)
...
var requiredFields = Set(["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"])   // "cid"
...
for passport in passports {
    ...
    print("")
    print(keys)
    print(keys.intersection(requiredFields))
