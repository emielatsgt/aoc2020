...
func state(in arrangement: Arrangement, x: Int, y: Int) -> State? {
    guard x >= 0 else { return nil }
    guard y >= 0 else { return nil }
    guard y < arrangement.count else { return nil }
    guard x < arrangement[y].count else { return nil }
    
    return arrangement[y][x]
}

func adjacent(in arrangement: Arrangement, x: Int, y: Int) -> Int {
    around
        .map { step -> State in
            var cx = x
            var cy = y
            ...
            repeat {
                cx += step[0]
                cy += step[1]
                current = state(in: arrangement, x: cx, y: cy)
...
func iterate(from arrangement: Arrangement) -> Arrangement {
    var newArrangement = arrangement
    
    for (y, _) in arrangement.enumerated() {
        for (x, _) in arrangement[y].enumerated() {
            switch state(in: arrangement, x: x, y: y) {
            case .occupied:
                newArrangement[y][x] = adjacent(in: arrangement, x: x, y: y) >= 5 ? .empty : .occupied
            case .empty:
                newArrangement[y][x] = adjacent(in: arrangement, x: x, y: y) == 0 ? .occupied : .empty
            case .floor:
                newArrangement[y][x] = .floor
            default: break
            }
        }
    }
...
func printArrangement(_ arrangement: Arrangement) {
    for line in arrangement {
        print(line.description.replacingOccurrences(of: ", ", with: ""))
    }
}
...
repeat {
    let newArrangement = iterate(from: lastArrangement)
    stable = newArrangement == lastArrangement
    lastArrangement = newArrangement
} while !stable

printArrangement(lastArrangement)

let occupied = lastArrangement
    .reduce(0) { accu, new in
       accu + new.reduce(0) { accu, new in new == .occupied ? accu + 1 : accu }
    }
