...
func state(in arrangement: Arrangement, x: Int, y: Int) -> State {
    guard x >= 0 else { return .floor }
    guard y >= 0 else { return .floor }
    guard y < arrangement.count else { return .floor }
    guard x < arrangement[y].count else { return .floor }
...
func adjacent(in arrangement: Arrangement, x: Int, y: Int) -> Int {
    around
        .map { state(in: arrangement, x: x + $0[0], y: y + $0[1]) }
        .filter { $0 == .occupied }
        .count
}
...
func iterate(from arrangement: Arrangement) -> Arrangement {
    ...
    for (y, _) in arrangement.enumerated() {
        for (x, _) in arrangement[y].enumerated() {
            ...
            case .occupied:
                newArrangement[y][x] = adjacent(in: arrangement, x: x, y: y) >= 4 ? .empty : .occupied
            case .empty:
