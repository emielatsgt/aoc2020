...
struct BagContent: CustomStringConvertible {
    let count: Int
    let color: String
...
struct Bag: CustomStringConvertible, Hashable, Equatable {
    let color: String
...
let bags = input
    ...
    .map { line -> Bag in
        ...
        let color = parts[0].trimmingCharacters(in: .whitespaces)
        
        let contentParts = parts[1]
            .components(separatedBy: ",")
            .map { $0.trimmingCharacters(in: .whitespaces) }
        
        let contents = contentParts.compactMap { part -> BagContent? in
            guard part != "no other" else { return nil }
            
            var parts = part.components(separatedBy: .whitespaces)
            let countString = parts.removeFirst()
            let color = parts.joined(separator: " ")
            let count = Int(countString)!
            
            return BagContent(count: count, color: color)
        }
        
        return Bag(color: color, contents: contents)
    }

func containers(for color: String) -> [Bag] {
    bags.filter { bag in
        !bag
            .contents
...
func containers(for bags: [Bag]) -> [Bag] {
    bags.reduce(into: [Bag]()) { accu, new in accu += containers(for: new.color) }
}

func recursiveContainers(for bags: [Bag]) -> [Bag] {
    guard !bags.isEmpty else { return [] }
    var temp = bags
    
    let new = containers(for: bags)
    temp += recursiveContainers(for: new)
    temp += new
...
func recurseBags(for bags: [Bag]) {
    for bag in bags {
        if !bag.contents.isEmpty {
            var otherBags: [Bag] = []
            for content in bag.contents {
                for _ in 0..<content.count {
                    otherBags.append(bagsByColor[content.color]!)
                }
            }
            
            recurseBags(for: otherBags)
        }
        
        for content in bag.contents {
            if let cc = colorCounts[content.color] {
                colorCounts[content.color] = cc + content.count
            } else {
                colorCounts[content.color] = content.count
            }
        }
    }
}

let bag = bagsByColor[myColor]!
