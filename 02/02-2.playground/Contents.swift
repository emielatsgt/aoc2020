...
for line in input.components(separatedBy: .newlines) {
    ...
    var left = parts[0]
    let password = parts[1].trimmingCharacters(in: .whitespaces)

    let character = left.removeLast()
    let numbers = left.components(separatedBy: "-")
        
    let first = Int(numbers[0].trimmingCharacters(in: .whitespaces))! - 1
    let second = Int(numbers[1].trimmingCharacters(in: .whitespaces))! - 1
    
    let hasFirst = password[password.index(password.startIndex, offsetBy: first)] == character
    let hasSecond = password[password.index(password.startIndex, offsetBy: second)] == character

    let isValid = hasFirst != hasSecond
