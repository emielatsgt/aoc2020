...
for line in input.components(separatedBy: .newlines) {
    ...
    let minimum = Int(numbers[0].trimmingCharacters(in: .whitespaces))!
    let maximum = Int(numbers[1].trimmingCharacters(in: .whitespaces))!
    
    let count = password.reduce(0) { accu, new in accu + (new == character ? 1 : 0) }
